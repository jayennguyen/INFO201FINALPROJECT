...
server <- function(input, output) {
  ...
  data5 <- reactive ({
    state <- input$state4
    region <- combine$vector[combine$states == state]
    return(region)
  })
...
output$plot4 <- renderPlot({
  us <- map_data('county')
  state <- subset(us, region == data5())
  base <- ggplot() + geom_polygon(data = state, mapping = aes(x = long, y = lat, group = group), color = 'white',  fill = "gray") +
    coord_fixed(1.3)
  point <- filter(data6(), INSTNM == input$college) 
   base <- base + geom_point(data = point, aes(x = as.numeric(LONGITUDE), y = as.numeric(LATITUDE)), color="red", size=10)
  return(base)
})
